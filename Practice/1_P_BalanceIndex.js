...
function balanceIndex(arr) {
    ...
    for (var i = 0; i < arr.length; i++) { // collect total sum of all elements in array
        arrTotal += arr[i];
    }
    for (var j = 0; j < arr.length - 1; j++) { // Loop through array again to check balance
        if (subSum === arrTotal - subSum - arr[j]) { // Check  the sum of some elements
            return j
        }
        subSum += arr[j];
    }
